    ...
    for env_value in instance_config_dict.get("env", {}).values():
        if is_secret_ref(env_value):
            ...
            if os.path.isfile(secret_file_name):
                ...
                paasta_print(failure(f"Secret file {secret_file_name} not defined", ""))
                return_value = False
    return return_value
...
def validate_secrets(service_path):
    ...
    for cluster in list_clusters(service, soa_dir):
        ...
        if not vault_env:
            paasta_print(failure(f"{cluster} not found on vault_cluster_map", ""))
            return_value = False
            continue
        ...
        for instance in list_all_instances_for_service(
            ...
            if not check_secrets_for_instance(
                ...
                return_value = False
    if return_value:
        paasta_print(success("No orphan secrets found"))
    return return_value
